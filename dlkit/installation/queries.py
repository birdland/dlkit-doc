...
class PackageQuery(osid_queries.OsidObjectQuery, osid_queries.OsidSourceableQuery):
    ...
    def match_any_requires_license_acknowledgement(self, match):
        ...
        :param match: ``true`` to match packages that have any acknowledgement value, ``false`` for to match packages that have no value
        :type match: ``boolean``
...
class InstallationContentQuery(osid_queries.OsidObjectQuery, osid_queries.OsidSubjugateableQuery):
    ...
    def get_installation_content_query_record(self, installation_content_record_type):
        """Gets the installation content query record corresponding to the given ``InstallationContent`` record ``Type``.
...
class DepotQuery(osid_queries.OsidCatalogQuery):
...
class InstallationQuery(osid_queries.OsidObjectQuery):
...
class SiteQuery(osid_queries.OsidObjectQuery):
    """This is the query for searching installations.
