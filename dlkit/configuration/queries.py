...
class ParameterQuery(osid_queries.OsidRuleQuery):
    ...
    def match_any_values_shuffled(self, match):
        ...
        :param match: ``true`` to match parameters with any shuffle value, ``false`` to match parameters with no shuffle value
        :type match: ``boolean``
...
class ValueQuery(osid_queries.OsidObjectQuery, osid_queries.OsidOperableQuery, osid_queries.OsidSubjugateableQuery):
...
class ConfigurationQuery(osid_queries.OsidCatalogQuery):
    ...
    def match_any_parameter(self, match):
        ...
        :param match: ``true`` to match configurations with any parameter, ``false`` to match configurations with no parameter
        :type match: ``boolean``
