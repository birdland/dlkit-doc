...
class Parameter(osid_objects.OsidRule):
    ...
    def implements_value_coordinate_type(self, coordinate_type):
        ...
        :return: ``true`` if the coordinate values associated with this parameter implement the given coordinate ``Type,``  ``false`` otherwise
        :rtype: ``boolean``
        :raise: ``IllegalState`` -- ``get_value_syntax() != coordinate``
    ...
    def implements_value_heading_type(self, heading_type):
        ...
        :return: ``true`` if the heading values associated with this parameter implement the given heading ``Type,``  ``false`` otherwise
        :rtype: ``boolean``
        :raise: ``IllegalState`` -- ``get_value_syntax() != heading``
    ...
    def implements_value_object_type(self, value_type):
        ...
        :return: ``true`` if the obect values associated with this parameter implement the given ``Type,``  ``false`` otherwise
        :rtype: ``boolean``
        :raise: ``IllegalState`` -- ``get_value_syntax() != object``
    ...
    def implements_value_spatial_unit_record_type(self, spatial_unit_record_type):
        ...
        :return: ``true`` if the spatial unit values associated with this parameter implement the given record ``Type,``  ``false`` otherwise
        :rtype: ``boolean``
        :raise: ``IllegalState`` -- ``get_value_syntax() != spatialunit``
    ...
    def implements_value_version_scheme(self, version_type):
        ...
        :return: ``true`` if the version values associated with this parameter implement the given version ``Type,``  ``false`` otherwise
        :rtype: ``boolean``
        :raise: ``IllegalState`` -- ``get_value_syntax() != version``
...
class ParameterForm(osid_objects.OsidRuleForm):
    ...
    value_syntax = property(fget=set_value_syntax, fdel=clear_value_syntax)
    ...
    value_coordinate_type = property(fget=set_value_coordinate_type, fdel=clear_value_coordinate_type)
    ...
    value_heading_type = property(fget=set_value_heading_type, fdel=clear_value_heading_type)
    ...
    value_object_type = property(fget=set_value_object_type, fdel=clear_value_object_type)
    ...
    value_spatial_unit_record_type = property(fget=set_value_spatial_unit_record_type, fdel=clear_value_spatial_unit_record_type)
    ...
    value_version_scheme = property(fget=set_value_version_scheme, fdel=clear_value_version_scheme)
    ...
    values_shuffled = property(fget=set_values_shuffled, fdel=clear_values_shuffled)
...
class ParameterList(osid_objects.OsidList):
    ...
    def get_next_parameter(self):
        ...
        :return: the next ``Parameter`` in this list. The ``has_next()`` method should be used to test that a next ``Parameter`` is available before calling this method.
    ...
    def get_next_parameters(self, n):
        ...
        :type n: ``cardinal``
        :return: an array of ``Parameter`` elements.The length of the array is less than or equal to the number specified.
        :rtype: ``osid.configuration.Parameter``
        :raise: ``IllegalState`` -- no more elements available in this list
        :raise: ``OperationFailed`` -- unable to complete request
...
class Value(osid_objects.OsidObject, osid_markers.Operable, osid_markers.Subjugateable):
...
class ValueForm(osid_objects.OsidObjectForm, osid_objects.OsidOperableForm, osid_objects.OsidSubjugateableForm):
...
class ValueList(osid_objects.OsidList):
    ...
    def get_next_value(self):
        ...
        :return: the next ``Value`` in this list. The ``has_next()`` method should be used to test that a next ``Value`` is available before calling this method.
...
class Configuration(osid_objects.OsidCatalog):
...
class ConfigurationForm(osid_objects.OsidCatalogForm):
...
class ConfigurationNode(osid_objects.OsidNode):
...
class ConfigurationNodeList(osid_objects.OsidList):
    ...
    def get_next_configuration_node(self):
        ...
        :return: the next ``ConfigurationNode`` in this list. The ``has_next()`` method should be used to test that a next ``ConfigurationNode`` is available before calling this method.
    ...
    def get_next_configuration_nodes(self, n):
        ...
        :param n: the number of ``ConfigurationNode`` elements requested which should be less than or equal to ``available()``
        :type n: ``cardinal``
        :return: an array of ``ConfigurationNode`` elements.The length of the array is less than or equal to the number specified.
        :rtype: ``osid.configuration.ConfigurationNode``
        :raise: ``IllegalState`` -- no more elements available in this list
        :raise: ``OperationFailed`` -- unable to complete request
