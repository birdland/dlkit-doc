...
class Proficiency(osid_objects.OsidRelationship):
...
class ProficiencyForm(osid_objects.OsidRelationshipForm):
...
class ProficiencyList(osid_objects.OsidList):
    ...
    def get_next_proficiency(self):
        ...
        :return: the next ``Proficiency`` in this list. The ``has_next()`` method should be used to test that a next
            ``Proficiency`` is available before calling this method.
    ...
    def get_next_proficiencies(self, n):
        ...
        :type n: ``cardinal``
        :return: an array of ``Proficiency`` elements.The length of the array is less than or equal to the number
            specified.
        :rtype: ``osid.learning.Proficiency``
        :raise: ``IllegalState`` -- no more elements available in this list
        :raise: ``OperationFailed`` -- unable to complete request
