"""Mongodb implementations of repository objects."""
...
class Asset(abc_repository_objects.Asset, osid_objects.OsidObject, osid_markers.Aggregateable, osid_markers.Sourceable):
    ...
    def get_provider_links(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        try:
            ...
            raise errors.OperationFailed('failed to instantiate ResourceManager')
        if not mgr.supports_resource_lookup():
            raise errors.OperationFailed('Resource does not support Resource lookup')
        lookup_session = mgr.get_resource_lookup_session()
        lookup_session.use_federated_bin_view()
    ...
    def get_asset_contents(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        return AssetContentList(self._my_map['assetContents'], runtime=self._runtime)
...
class AssetForm(abc_repository_objects.AssetForm, osid_objects.OsidObjectForm, osid_objects.OsidAggregateableForm, osid_objects.OsidSourceableForm):
    ...
    def _init_metadata(self, **kwargs):
        ...
        self._copyright_registration_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'copyright_registration')}
        self._copyright_registration_metadata.update(mdata_conf.ASSET_COPYRIGHT_REGISTRATION)
        self._copyright_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'copyright')}
        self._copyright_metadata.update(mdata_conf.ASSET_COPYRIGHT)
        self._title_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'title')}
        self._title_metadata.update(mdata_conf.ASSET_TITLE)
        self._distribute_verbatim_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'distribute_verbatim')}
        self._distribute_verbatim_metadata.update(mdata_conf.ASSET_DISTRIBUTE_VERBATIM)
        self._created_date_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'created_date')}
        self._created_date_metadata.update(mdata_conf.ASSET_CREATED_DATE)
        self._distribute_alterations_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'distribute_alterations')}
        self._distribute_alterations_metadata.update(mdata_conf.ASSET_DISTRIBUTE_ALTERATIONS)
        self._principal_credit_string_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'principal_credit_string')}
        self._principal_credit_string_metadata.update(mdata_conf.ASSET_PRINCIPAL_CREDIT_STRING)
        self._published_date_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'published_date')}
        self._published_date_metadata.update(mdata_conf.ASSET_PUBLISHED_DATE)
        self._source_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'source')}
        self._source_metadata.update(mdata_conf.ASSET_SOURCE)
        self._provider_links_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'provider_links')}
        self._provider_links_metadata.update(mdata_conf.ASSET_PROVIDER_LINKS)
        self._public_domain_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'public_domain')}
        self._public_domain_metadata.update(mdata_conf.ASSET_PUBLIC_DOMAIN)
        self._distribute_compositions_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'distribute_compositions')}
        self._distribute_compositions_metadata.update(mdata_conf.ASSET_DISTRIBUTE_COMPOSITIONS)
        self._composition_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'composition')}
        self._composition_metadata.update(mdata_conf.ASSET_COMPOSITION)
        self._published_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'published')}
        self._published_metadata.update(mdata_conf.ASSET_PUBLISHED)
        self._copyright_registration_default = self._copyright_registration_metadata['default_string_values'][0]
        self._copyright_default = dict(self._copyright_metadata['default_string_values'][0])
        self._title_default = dict(self._title_metadata['default_string_values'][0])
        self._distribute_verbatim_default = None
        self._created_date_default = self._created_date_metadata['default_date_time_values'][0]
        self._distribute_alterations_default = None
        self._principal_credit_string_default = dict(self._principal_credit_string_metadata['default_string_values'][0])
        self._published_date_default = self._published_date_metadata['default_date_time_values'][0]
        self._source_default = self._source_metadata['default_id_values'][0]
        self._provider_links_default = self._provider_links_metadata['default_id_values']
        self._public_domain_default = None
        self._distribute_compositions_default = None
        self._composition_default = self._composition_metadata['default_id_values'][0]
        self._published_default = None
    ...
    def get_title_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        metadata = dict(self._title_metadata)
        metadata.update({'existing_title_values': self._my_map['title']})
    ...
    def set_title(self, title):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if self.get_title_metadata().is_read_only():
            raise errors.NoAccess()
        if not self._is_valid_string(title, self.get_title_metadata()):
    ...
    def get_public_domain_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._public_domain_metadata)
        metadata.update({'existing_public_domain_values': self._my_map['publicDomain']})
        return Metadata(**metadata)
    ...
    def get_copyright_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._copyright_metadata)
        metadata.update({'existing_copyright_values': self._my_map['copyright']})
        return Metadata(**metadata)
    ...
    def set_copyright(self, copyright_):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.repository.AssetForm.set_title_template
        if self.get_copyright_metadata().is_read_only():
            raise errors.NoAccess()
        if not self._is_valid_string(copyright_, self.get_copyright__metadata()):
            raise errors.InvalidArgument()
        self._my_map['copyright']['text'] = copyright_
    ...
    def get_copyright_registration_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._copyright_registration_metadata)
        metadata.update({'existing_copyright_registration_values': self._my_map['copyrightRegistration']})
        return Metadata(**metadata)
    ...
    def get_distribute_verbatim_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._distribute_verbatim_metadata)
        metadata.update({'existing_distribute_verbatim_values': self._my_map['distributeVerbatim']})
        return Metadata(**metadata)
    ...
    def get_distribute_alterations_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._distribute_alterations_metadata)
        metadata.update({'existing_distribute_alterations_values': self._my_map['distributeAlterations']})
        return Metadata(**metadata)
    ...
    def get_distribute_compositions_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._distribute_compositions_metadata)
        metadata.update({'existing_distribute_compositions_values': self._my_map['distributeCompositions']})
        return Metadata(**metadata)
    ...
    def get_source_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        metadata.update({'existing_source_values': self._my_map['sourceId']})
        return Metadata(**metadata)
    ...
    def get_provider_links_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        metadata = dict(self._provider_links_metadata)
    ...
    def get_created_date_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._created_date_metadata)
        metadata.update({'existing_created_date_values': self._my_map['createdDate']})
        return Metadata(**metadata)
    ...
    def get_published_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._published_metadata)
        metadata.update({'existing_published_values': self._my_map['published']})
        return Metadata(**metadata)
    ...
    def get_published_date_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._published_date_metadata)
        metadata.update({'existing_published_date_values': self._my_map['publishedDate']})
        return Metadata(**metadata)
    ...
    def get_principal_credit_string_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._principal_credit_string_metadata)
        metadata.update({'existing_principal_credit_string_values': self._my_map['principalCreditString']})
        return Metadata(**metadata)
    ...
    def set_principal_credit_string(self, credit_string):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.repository.AssetForm.set_title_template
        if self.get_principal_credit_string_metadata().is_read_only():
            raise errors.NoAccess()
        if not self._is_valid_string(credit_string, self.get_credit_string_metadata()):
            raise errors.InvalidArgument()
    ...
    def get_composition_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._composition_metadata)
        metadata.update({'existing_composition_values': self._my_map['compositionId']})
        return Metadata(**metadata)
...
class AssetContent(abc_repository_objects.AssetContent, osid_objects.OsidObject, osid_markers.Subjugateable):
    ...
    def __init__(self, osid_object_map, runtime=None):
        osid_objects.OsidObject.__init__(self, osid_object_map, runtime)
        self._record_type_data_sets = self._get_registry('ASSET_CONTENT_RECORD_TYPES')
        self._records = dict()
        self._load_records(osid_object_map['recordTypeIds'])
        self._catalog_name = 'repository'
...
class AssetContentForm(abc_repository_objects.AssetContentForm, osid_objects.OsidObjectForm, osid_objects.OsidSubjugateableForm):
    ...
    def _init_metadata(self, **kwargs):
        ...
        self._url_metadata = {
            'element_id': Id(
                ...
                self._namespace,
                'url')}
        self._url_metadata.update(mdata_conf.ASSET_CONTENT_URL)
        self._data_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'data')}
        self._data_metadata.update(mdata_conf.ASSET_CONTENT_DATA)
        self._accessibility_type_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'accessibility_type')}
        self._accessibility_type_metadata.update(mdata_conf.ASSET_CONTENT_ACCESSIBILITY_TYPE)
        self._url_default = self._url_metadata['default_string_values'][0]
        self._data_default = self._data_metadata['default_object_values'][0]
        self._accessibility_type_default = self._accessibility_type_metadata['default_type_values'][0]
    ...
    def get_accessibility_type_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._accessibility_type_metadata)
        metadata.update({'existing_accessibility_type_values': self._my_map['accessibilityType']})
        return Metadata(**metadata)
    ...
    def get_data_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._data_metadata)
        metadata.update({'existing_data_values': self._my_map['data']})
        return Metadata(**metadata)
    ...
    def get_url_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._url_metadata)
        metadata.update({'existing_url_values': self._my_map['url']})
        return Metadata(**metadata)
...
class Composition(abc_repository_objects.Composition, osid_objects.OsidObject, osid_markers.Containable, osid_markers.Operable, osid_markers.Sourceable):
    ...
    def __init__(self, osid_object_map, runtime=None):
        osid_objects.OsidObject.__init__(self, osid_object_map, runtime)
        self._record_type_data_sets = self._get_registry('COMPOSITION_RECORD_TYPES')
        self._records = dict()
        self._load_records(osid_object_map['recordTypeIds'])
        self._catalog_name = 'repository'
    ...
    def get_children(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        try:
            ...
            raise errors.OperationFailed('failed to instantiate RepositoryManager')
        if not mgr.supports_composition_lookup():
            raise errors.OperationFailed('Repository does not support Composition lookup')
        lookup_session = mgr.get_composition_lookup_session()
        lookup_session.use_federated_repository_view()
...
class CompositionForm(abc_repository_objects.CompositionForm, osid_objects.OsidObjectForm, osid_objects.OsidContainableForm, osid_objects.OsidOperableForm, osid_objects.OsidSourceableForm):
    ...
    def _init_metadata(self, **kwargs):
        ...
        self._children_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'children')}
        self._children_metadata.update(mdata_conf.COMPOSITION_CHILDREN)
        self._children_default = self._children_metadata['default_id_values']
...
class Repository(abc_repository_objects.Repository, osid_objects.OsidCatalog):
    ...
    def __init__(self, osid_catalog_map, runtime=None):
        ...
        self._record_type_data_sets = self._get_registry('REPOSITORY_RECORD_TYPES')
        self._records = dict()
    ...
    def get_repository(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if self._lookup_session is None:
            ...
            self._lookup_session = mgr.get_repository_lookup_session()
