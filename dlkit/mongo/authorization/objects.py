"""Mongodb implementations of authorization objects."""
...
class Authorization(abc_authorization_objects.Authorization, osid_objects.OsidRelationship):
    ...
    def get_agent(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        lookup_session = mgr.get_agent_lookup_session()
        lookup_session.use_federated_agency_view()
    ...
    def get_function(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        lookup_session = mgr.get_function_lookup_session()
...
class AuthorizationList(abc_authorization_objects.AuthorizationList, osid_objects.OsidList):
    """Like all ``OsidLists,``  ``AuthorizationList`` provides a means for accessing ``Authorization`` elements
        sequentially
    ...
    def get_next_authorizations(self, n):
        """Gets the next set of ``Authorization`` elements in this list which must be less than or equal to the number
...
class Vault(abc_authorization_objects.Vault, osid_objects.OsidCatalog):
    ...
    def __init__(self, osid_catalog_map, runtime=None):
        ...
        self._record_type_data_sets = self._get_registry('VAULT_RECORD_TYPES')
        self._records = dict()
...
class VaultList(abc_authorization_objects.VaultList, osid_objects.OsidList):
    """Like all ``OsidLists,``  ``VaultList`` provides a means for accessing ``Vault`` elements sequentially either one
        at a
    ...
    def get_next_vaults(self, n):
        """Gets the next set of ``Vault`` elements in this list which must be less than or equal to the return from
            ``available()``.
    ...
    def get_vault(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if self._lookup_session is None:
            ...
            self._lookup_session = mgr.get_vault_lookup_session()
...
class VaultNodeList(abc_authorization_objects.VaultNodeList, osid_objects.OsidList):
    """Like all ``OsidLists,``  ``VaultNodeList`` provides a means for accessing ``VaultNode`` elements sequentially
        either one
    ...
    def get_next_vault_nodes(self, n):
        """Gets the next set of ``VaultNode`` elements in this list which must be less than or equal to the return from
            ``available()``.
