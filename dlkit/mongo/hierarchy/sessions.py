"""Mongodb implementations of hierarchy sessions."""
...
from ...abstract_osid.hierarchy.objects import HierarchyForm as ABCHierarchyForm
...
class HierarchyTraversalSession(abc_hierarchy_sessions.HierarchyTraversalSession, osid_sessions.OsidSession):
    ...
    def __init__(self, catalog_id=None, proxy=None, runtime=None, **kwargs):
        ...
        OsidSession._init_object(self, catalog_id, proxy, runtime, db_name='hierarchy', cat_name='Hierarchy',
            cat_class=Hierarchy)
        self._kwargs = kwargs
    ...
    def _set_relationship_type(self, type_identifier, display_name=None, display_label=None, description=None,
        domain='Relationship'):
...
class HierarchyDesignSession(abc_hierarchy_sessions.HierarchyDesignSession, osid_sessions.OsidSession):
    ...
    def __init__(self, catalog_id=None, proxy=None, runtime=None, **kwargs):
        ...
        self._hts = HierarchyTraversalSession(self.get_hierarchy_id(), self._proxy, self._runtime,
    ...
    def add_root(self, id_):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if (bool(self._rls.get_relationships_by_genus_type_for_source(id_, self._relationship_type).available()) or
                bool(self._rls.get_relationships_by_genus_type_for_destination(id_,
                    self._relationship_type).available())):
    ...
    def add_child(self, id_, child_id):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        if bool(self._rls.get_relationships_by_genus_type_for_peers(id_, child_id,
            self._relationship_type).available()):
            raise errors.AlreadyExists()
        ...
        rfc.set_description(self._relationship_type.get_display_name().get_text() + ' relationship for parent: ' +
            str(id_) + ' and child: ' + str(child_id))
    ...
    def remove_root(self, id_):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        result = self._rls.get_relationships_by_genus_type_for_peers(self._phantom_root_id, id_,
...
class HierarchyLookupSession(abc_hierarchy_sessions.HierarchyLookupSession, osid_sessions.OsidSession):
    ...
    _session_name = 'HierarchyLookupSession'
    ...
    def get_hierarchy(self, hierarchy_id):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if hierarchy_id.get_identifier() == '000000000000000000000000':
            return self._get_phantom_root_catalog(cat_class=objects.Hierarchy, cat_name='Hierarchy')
        try:
            result = collection.find_one({'_id': ObjectId(hierarchy_id.get_identifier())})
        except errors.NotFound:
            # Try creating an orchestrated Hierarchy.  Let it raise errors.NotFound()
            result = self._create_orchestrated_cat(hierarchy_id, 'hierarchy', 'Hierarchy')


        return objects.Hierarchy(result, runtime=self._runtime)
    ...
    def get_hierarchies_by_ids(self, hierarchy_ids):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        # Also, this should be implemented to use get_Hierarchy() instead of direct to database
        catalog_id_list = []
        for i in hierarchy_ids:
        ...
        return objects.HierarchyList(result, runtime=self._runtime)
...
class HierarchyAdminSession(abc_hierarchy_sessions.HierarchyAdminSession, osid_sessions.OsidSession):
    ...
    _session_name = 'HierarchyAdminSession'
    ...
    def create_hierarchy(self, hierarchy_form):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        try:
            if self._forms[hierarchy_form.get_id().get_identifier()] == CREATED:
                raise errors.IllegalState('hierarchy_form already used in a create transaction')
        except KeyError:
            raise errors.Unsupported('hierarchy_form did not originate from this session')
        if not hierarchy_form.is_valid():
            raise errors.InvalidArgument('one or more of the form elements is invalid')
        insert_result = collection.insert_one(hierarchy_form._my_map)


        self._forms[hierarchy_form.get_id().get_identifier()] = CREATED
        result = objects.Hierarchy(
    ...
    def get_hierarchy_form_for_update(self, hierarchy_id):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        cat_form = objects.HierarchyForm(result, runtime=self._runtime)
    ...
    def update_hierarchy(self, hierarchy_form):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        return objects.Hierarchy(hierarchy_form._my_map, runtime=self._runtime)
