"""Mongodb implementations of commenting objects."""
...
class Comment(abc_commenting_objects.Comment, osid_objects.OsidRelationship):
    ...
    def __init__(self, osid_object_map, runtime=None):
        ...
        self._record_type_data_sets = self._get_registry('COMMENT_RECORD_TYPES')
        self._records = dict()
...
class CommentForm(abc_commenting_objects.CommentForm, osid_objects.OsidRelationshipForm):
    ...
    def _init_metadata(self, **kwargs):
        ...
        self._text_metadata = {
            'element_id': Id(
                ...
                self._namespace,
                'text')}
        self._text_metadata.update(mdata_conf.COMMENT_TEXT)
        self._rating_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'rating')}
        self._rating_metadata.update(mdata_conf.COMMENT_RATING)
        self._text_default = dict(self._text_metadata['default_string_values'][0])
        self._rating_default = self._rating_metadata['default_id_values'][0]
    ...
    def get_text_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        metadata = dict(self._text_metadata)
        metadata.update({'existing_text_values': self._my_map['text']})
    ...
    def set_text(self, text):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if not self._is_valid_string(text, self.get_text_metadata()):
            raise errors.InvalidArgument()
        self._my_map['text']['text'] = text
    ...
    def get_rating_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._rating_metadata)
        metadata.update({'existing_rating_values': self._my_map['ratingId']})
        return Metadata(**metadata)
...
class Book(abc_commenting_objects.Book, osid_objects.OsidCatalog):
    ...
    def __init__(self, osid_catalog_map, runtime=None):
        ...
        self._record_type_data_sets = self._get_registry('BOOK_RECORD_TYPES')
        self._records = dict()
    ...
    def get_book(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if self._lookup_session is None:
            ...
            self._lookup_session = mgr.get_book_lookup_session()
