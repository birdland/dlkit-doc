"""Mongodb implementations of learning objects."""
...
class Objective(abc_learning_objects.Objective, osid_objects.OsidObject, osid_markers.Federateable):
    ...
    def __init__(self, osid_object_map, runtime=None):
        ...
        self._record_type_data_sets = self._get_registry('OBJECTIVE_RECORD_TYPES')
        self._records = dict()
...
class ObjectiveForm(abc_learning_objects.ObjectiveForm, osid_objects.OsidObjectForm, osid_objects.OsidFederateableForm):
    ...
    def _init_metadata(self, **kwargs):
        ...
        self._cognitive_process_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'cognitive_process')}
        self._cognitive_process_metadata.update(mdata_conf.OBJECTIVE_COGNITIVE_PROCESS)
        self._assessment_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'assessment')}
        self._assessment_metadata.update(mdata_conf.OBJECTIVE_ASSESSMENT)
        self._knowledge_category_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'knowledge_category')}
        self._knowledge_category_metadata.update(mdata_conf.OBJECTIVE_KNOWLEDGE_CATEGORY)
        self._cognitive_process_default = self._cognitive_process_metadata['default_id_values'][0]
        self._assessment_default = self._assessment_metadata['default_id_values'][0]
        self._knowledge_category_default = self._knowledge_category_metadata['default_id_values'][0]
    ...
    def get_assessment_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        metadata = dict(self._assessment_metadata)
        metadata.update({'existing_assessment_values': self._my_map['assessmentId']})
    ...
    def get_knowledge_category_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._knowledge_category_metadata)
        metadata.update({'existing_knowledge_category_values': self._my_map['knowledgeCategoryId']})
        return Metadata(**metadata)
    ...
    def get_cognitive_process_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._cognitive_process_metadata)
        metadata.update({'existing_cognitive_process_values': self._my_map['cognitiveProcessId']})
        return Metadata(**metadata)
...
class Activity(abc_learning_objects.Activity, osid_objects.OsidObject, osid_markers.Subjugateable):
    ...
    def __init__(self, osid_object_map, runtime=None):
        osid_objects.OsidObject.__init__(self, osid_object_map, runtime)
        self._record_type_data_sets = self._get_registry('ACTIVITY_RECORD_TYPES')
        self._records = dict()
        self._load_records(osid_object_map['recordTypeIds'])
        self._catalog_name = 'objective_bank'
    ...
    def get_courses(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        try:
            ...
            raise errors.OperationFailed('failed to instantiate CourseManager')
        if not mgr.supports_course_lookup():
            raise errors.OperationFailed('Course does not support Course lookup')
        lookup_session = mgr.get_course_lookup_session()
    ...
    def get_assessments(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        try:
            ...
            raise errors.OperationFailed('failed to instantiate AssessmentManager')
        if not mgr.supports_assessment_lookup():
            raise errors.OperationFailed('Assessment does not support Assessment lookup')
        lookup_session = mgr.get_assessment_lookup_session()
        lookup_session.use_federated_bank_view()
...
class ActivityForm(abc_learning_objects.ActivityForm, osid_objects.OsidObjectForm, osid_objects.OsidSubjugateableForm):
    ...
    def _init_metadata(self, **kwargs):
        osid_objects.OsidObjectForm._init_metadata(self, **kwargs)
        self._courses_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'courses')}
        self._courses_metadata.update(mdata_conf.ACTIVITY_COURSES)
        self._assessments_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'assessments')}
        self._assessments_metadata.update(mdata_conf.ACTIVITY_ASSESSMENTS)
        self._assets_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'assets')}
        self._assets_metadata.update(mdata_conf.ACTIVITY_ASSETS)
        self._courses_default = self._courses_metadata['default_id_values']
        self._assessments_default = self._assessments_metadata['default_id_values']
        self._assets_default = self._assets_metadata['default_id_values']
    ...
    def get_assets_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        metadata = dict(self._assets_metadata)
    ...
    def get_courses_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.learning.ActivityForm.get_assets_metadata_template
        metadata = dict(self._courses_metadata)
    ...
    def get_assessments_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.learning.ActivityForm.get_assets_metadata_template
        metadata = dict(self._assessments_metadata)
...
class Proficiency(abc_learning_objects.Proficiency, osid_objects.OsidRelationship):
    ...
    def __init__(self, osid_object_map, runtime=None):
        osid_objects.OsidObject.__init__(self, osid_object_map, runtime)
        self._record_type_data_sets = self._get_registry('PROFICIENCY_RECORD_TYPES')
        self._records = dict()
        self._load_records(osid_object_map['recordTypeIds'])
        self._catalog_name = 'objective_bank'
...
class ProficiencyForm(abc_learning_objects.ProficiencyForm, osid_objects.OsidRelationshipForm):
    ...
    def _init_metadata(self, **kwargs):
        ...
        self._completion_metadata = {
            'element_id': Id(
                ...
                self._namespace,
                'completion')}
        self._completion_metadata.update(mdata_conf.PROFICIENCY_COMPLETION)
        self._level_metadata = {
            'element_id': Id(
                self._authority,
                self._namespace,
                'level')}
        self._level_metadata.update(mdata_conf.PROFICIENCY_LEVEL)
        self._completion_default = self._completion_metadata['default_decimal_values'][0]
    ...
    def _init_map(self, **kwargs):
        ...
        self._my_map['level'] = self._level_metadata['default_id_values'][0]
    ...
    def get_completion_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        # Implemented from template for osid.resource.ResourceForm.get_group_metadata_template
        metadata = dict(self._completion_metadata)
        metadata.update({'existing_completion_values': self._my_map['completion']})
        return Metadata(**metadata)
    ...
    def get_level_metadata(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        metadata.update({'existing_level_values': self._my_map['level']})
        return Metadata(**metadata)
...
class ProficiencyList(abc_learning_objects.ProficiencyList, osid_objects.OsidList):
    """Like all ``OsidLists,``  ``ProficiencyList`` provides a means for accessing ``Proficiency`` elements sequentially
        either
...
class ObjectiveBank(abc_learning_objects.ObjectiveBank, osid_objects.OsidCatalog):
    ...
    def __init__(self, osid_catalog_map, runtime=None):
        ...
        self._record_type_data_sets = self._get_registry('OBJECTIVE_BANK_RECORD_TYPES')
        self._records = dict()
    ...
    def get_objective_bank(self):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if self._lookup_session is None:
            ...
            self._lookup_session = mgr.get_objective_bank_lookup_session()
