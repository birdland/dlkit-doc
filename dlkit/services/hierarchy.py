...
# The following methods are from osid.hierarchy.HierarchyLookupSession
    ...
    def use_comparative_hierarchy_view(self):
        ...
        for session in self._provider_sessions:
            try:
                self._provider_sessions[session].use_comparative_hierarchy_view()
            except AttributeError:
                pass
    ...
    def use_plenary_hierarchy_view(self):
        ...
        for session in self._provider_sessions:
            try:
                self._provider_sessions[session].use_plenary_hierarchy_view()
            except AttributeError:
                pass
    ...
    def get_hierarchies_by_ids(self, *args, **kwargs):
        ...
        cat_list = []
        for cat in catalogs:
            cat_list.append(Hierarchy(self._provider_manager, cat, self._proxy))
...
# The following methods are from osid.hierarchy.HierarchyAdminSession
    ...
    def create_hierarchy(self, *args, **kwargs):
        ...
        return Hierarchy(self._provider_manager, self._get_provider_session('hierarchy_admin_session').create_hierarchy(*args, **kwargs), self._proxy)
    ...
    def update_hierarchy(self, *args, **kwargs):
        ...
        return Hierarchy(self._provider_manager, self._get_provider_session('hierarchy_admin_session').update_hierarchy(*args, **kwargs), self._proxy)
